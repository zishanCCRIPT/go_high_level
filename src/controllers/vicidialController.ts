...
export const addLead = async (req: Request, res: Response): Promise<any> => {
  ...
  if (!sanitizedPhoneNumber || sanitizedPhoneNumber.length < 6 || sanitizedPhoneNumber.length > 16) {
...
export const updateLead = async (req: Request, res: Response): Promise<any> => {
  ...
  try {
    ...
    if (street_address) params.append('address1', street_address.substring(0, 100));
    ...
    if (country_code) params.append('country_code', country_code.substring(0, 3));
